...
source "amazon-ebs" "autogenerated_1" {
  ami_name      = "DevSecOps-B38-Ansible-{{ clean_resource_name `${timestamp()}` }}"
  ...
  tags = {
    Name = "DevSecOps-B38-Ansible-{{ clean_resource_name `${timestamp()}` }}"
  }
  vpc_id = "${var.vpc_id}"
}
...
build {
  ...
  provisioner "shell" {
    inline = ["sudo cp /tmp/docker.service /lib/systemd/system/docker.service", "sudo usermod -a -G docker ubuntu", "sudo usermod -a -G docker ansibleadmin", "sudo systemctl daemon-reload", "sudo service docker restart"]
  }
